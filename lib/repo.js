...
function repo (options, callback) {
  ...
  if (path) {
    ...
    contentsUrl += '?' + qs.stringify({ access_token: token, ref: branch })
    ...
    return request(contentsUrl, function (err, data) {
      ...
      if (isDir) {
        ...
        url += '?' + qs.stringify({ access_token: token, ref: branch })
        ...
        return request(url, function (err, json) {
          ...
          if (files.length) {
            var filesUrl = 'https://raw.github.com/' + user + '/' + repo + '/' + branch + '/' + path + '/'
            return repoFiles({
              url: filesUrl,
              files: files
            }, callback)
      ...
      urls[0] += '?' + qs.stringify({ access_token: token, ref: branch })
  ...
  request(url, function (err, json) {
    ...
    if (files.length) {
      var filesUrl = 'https://raw.github.com/' + user + '/' + repo + '/' + branch + '/'
      return repoFiles({
        url: filesUrl,
        files: files
      }, callback)
